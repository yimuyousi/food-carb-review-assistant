...
- 可以额外读取本地私有项目上下文，用于未公开写作项目，但不会把该上下文上传到 GitHub。
...
`references/project_context_template.md`

本地私有项目上下文模板。需要服务某个具体写作项目时，把它复制到 `.private/project_context.local.md`，然后只在本机填写具体题目、章节、表格和写作要求。
...
公开上传 GitHub 的 skill 保持通用。你自己写具体项目时，使用本地私有文件：

```text
.private/project_context.local.md
```

这个文件已经被 `.gitignore` 忽略，不会被正常提交到 GitHub。使用时可以对 OpenClaw 说：

```text
请使用 polysaccharide-research-assistant skill，并加载我的本地私有项目上下文，帮我起草这一节。
```

这样做的效果是：公开 skill 仍然是通用多糖研究助手；你本地使用时，它可以按你的具体章节、表格和写作逻辑来工作。
